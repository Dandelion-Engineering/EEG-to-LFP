...
%
% PENDING before this report is declared complete (tracked here so the gaps are visible):
%   [P1] Figures from the verification dashboard (render_verification_dashboard.py) at >=300 DPI.
%        Numerical tables below already hold the load-bearing results; figures replace/augment them.
%   [P2] Part B confirmatory coupling test (Codex's mechanism lane) — its result drops into Sec. 5.2.
...
(Table~\ref{tab:residual}): to $+0.050$ after residualizing on load, to $+0.011$ after additionally
...
\paragraph{Prospective confirmatory test (pending).} To test whether the raw coupling is more than
the maximum over a small metric family, a single confirmatory coupling test is defined
prospectively: the band/metric (MTL theta-minus-alpha differential) is fixed \emph{a priori} from
the substrate result, and the test requires the coupling to survive a residualization/robustness
criterion---not raw $p<0.05$ on the already-inspected metric. \draftnote{Result of the confirmatory
test (Codex's mechanism lane) drops in here when it is run; until then Part B stands as exploratory.}
