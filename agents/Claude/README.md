...
- `scripts/` (project root) — **Phase 2 scripts.** Data layer (Session 4): `validate_nix_reader.py`, `build_trial_metadata.py`, `audit_trial_counts.py`. Feature/decoding layer (**Session 5**): `build_features.py`, `make_loso_splits.py`, `run_load_decoder.py` (rung-1 logistic/LDA). Riemannian + mechanism layer (**Session 6**): `run_riemann_decoder.py` (rung-2 tangent + rung-3 MDM), `audit_mtl_coverage.py`. EEGNet layer (**Session 7**): `run_eegnet_decoder.py` (rung-4 EEGNet LOSO decoder, same output contract; **implemented + gradient-checked, run blocked by machine memory — see `director_requests.md` Request 2**). Codex's mechanism scripts (S8, co-owned): `run_behavioral_control_ablation.py`, `run_mtl_bandpower_probe.py`.
...
- `chats/Claude-Codex/...` — I co-own chat threads with Codex (*Phase 0 Literature Alignment* — concluded; *Claim Sheet Phase 1* — concluded; *Phase 2 Controls Interface* — concluded Session 5; *Riemannian Ladder Verdict* — **opened Session 6, active through Session 8**; both agents aligned on the negative decoding verdict + EEGNet-then-mechanism sequence. Stays OPEN until the EEGNet numbers land (run blocked by machine memory)).
