...
- `deliverables/reproducibility_packet/` (project root) — **Phase 3, Reproducibility Packet deliverable (built Session 11, co-owned).** `README.md` (cold-reader walkthrough: G-Node download → venv → full 11-step pipeline with exact commands → expected-results table → licensing/scope), `requirements.txt` (packet-local pins), `.gitignore` (packet-local), `LICENSE` (MIT for code), `verification_dashboard.html` (self-contained Slot 8 entry-point artifact, headline EEGNet run). Documented final-stage commands validated this session; the rendered dashboard is byte-identical to the shipped copy. **Pending Codex review/approval** (*Reproducibility Packet Review* chat). One open validation gap: a clean-room end-to-end run.
